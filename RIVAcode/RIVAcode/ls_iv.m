function [A_id,B_id,norm_est_err_dy,sol_found,Meas_mtx_sim,Err_mtx]...
    = ls_iv(Meas_mtx,Meas_mtx_sim_pr,null_constr_list,num_tps_4exp)

% The function computes the connectivity matrix A and exogenous
% perturbation matrix B by using the IV technique, with imposing the 
% desired zero entries of the connectivity matrix A and of the input matrix 
% B contained in the list named null_constr_list (if no information is 
% available for A and B, then the list is empty).
...
%                   
% Meas_mtx_sim:     Simulated measurements matrix, each column contains the 
...
% Matrixes of the measurements at time point k from 1 to n_time_pts-1
Omega_mtx=[];
IV_mtx=[];
...
eig_Rec_mtx=eig(A_id);
mod_eig_val=abs(eig_Rec_mtx);
...
for num_ts=1:num_exp
    ...
    ind_tps_end=sum(num_tps_4exp(1:num_ts))-num_ts;
    %Meas_mtx_r=zeros(n_time_pts_exp,n_nodes);
