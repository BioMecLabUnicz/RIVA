...
% dynGenie3 (Figures 5 and 6, cases g) h) and i)).
...
% emtab_1908_L_643_15_res.mat, emtab_1908_L_643_10_res.mat or 
% emtab_1908_L_643_8_res.mat 
...
if not(do_sim)
    ...
    filename= ['Bio_Example_data_results/results/emtab_1908_L_643_',num2str(val_ts),'_res.mat'];
    load  (filename)
    ...
    for num_net=2
        ...
        for num_test=1:n_tests
            ...
            eval (['Meas_mtx_1=',str_array,'{num_net,num_test};']);
            
            eval (['Meas_mtx_2=',str_array_2,'{1,num_test};']);
            
          
            Meas_mtx_1=Meas_mtx_1';
            Meas_mtx_1=Meas_mtx_1(idx_time1:idx_time2,:);
              
            Meas_mtx_2=Meas_mtx_2';
            Meas_mtx=[Meas_mtx_1;Meas_mtx_2];
            ...
            num_tps_4exp(1)=size(Meas_mtx_1,1);
            num_tps_4exp(2)=size(Meas_mtx_2,1);
         ...
         eval (['save Bio_Example_data_results/results/emtab_1908_L_643_',num2str(val_ts),'_redo_res.mat'])
...
num_net=2;
% Riva performance
AUROC_val=(perf_auroc_RLS_IV(num_net,:));
                        
AUPR_val=(perf_aupr_RLS_IV(num_net,:));
